...
class Volume {
  ...
  constructor(
    ...
    Object.defineProperty(this, "lowerThreshold", {
      ...
      set: function (value) {
        ...
        this.sliceList.forEach(function (slice: any) {
  ...
  getData(i: number, j: number, k: number) {
    return (this.data as any)[
      k * (this.xLength as number) * (this.yLength as number) +
        j * (this.xLength as number) +
        i
    ];
  }
  ...
  reverseAccess(index: number) {
    const z = Math.floor(
      index / ((this.yLength as number) * (this.xLength as number))
    );
    const y = Math.floor(
      (index - z * (this.yLength as number) * (this.xLength as number)) /
        (this.xLength as number)
    );
    const x =
      index -
      z * (this.yLength as number) * (this.xLength as number) -
      y * (this.xLength as number);
    return [x, y, z];
  }
   ...
   * @returns {Volume}   this
   */
  map(functionToMap: any, context: any) {
    const length = (this.data as any).length;
    context = context || this;

    for (let i = 0; i < length; i++) {
      (this.data as any)[i] = functionToMap.call(
        context,
        (this.data as any)[i],
        i,
        this.data
  ...
  extractPerpendicularPlane(axis: string, RASIndex: number) {
    let firstSpacing, secondSpacing, positionOffset, IJKIndex: any;
    ...
    switch (axis) {
      case "x":
        ...
        firstSpacing = this.spacing[(this.axisOrder as string[]).indexOf("z")];
        secondSpacing = this.spacing[(this.axisOrder as string[]).indexOf("y")];
        IJKIndex = new Vector3(RASIndex, 0, 0);

        planeMatrix.multiply(new Matrix4().makeRotationY(Math.PI / 2));
        positionOffset = ((volume as any).RASDimensions[0] - 1) / 2;
      ...
      case "y":
        ...
        firstSpacing = this.spacing[(this.axisOrder as string[]).indexOf("x")];
        secondSpacing = this.spacing[(this.axisOrder as string[]).indexOf("z")];
        IJKIndex = new Vector3(0, RASIndex, 0);

        planeMatrix.multiply(new Matrix4().makeRotationX(-Math.PI / 2));
        positionOffset = ((volume as any).RASDimensions[1] - 1) / 2;
      ...
      default:
        ...
        positionOffset = ((volume as any).RASDimensions[2] - 1) / 2;
        planeMatrix.setPosition(new Vector3(0, 0, RASIndex - positionOffset));
        break;
    }

    firstDirection.applyMatrix4((volume as any).inverseMatrix).normalize();
    (firstDirection as any).arglet = "i";
    secondDirection.applyMatrix4((volume as any).inverseMatrix).normalize();
    (firstDirection as any).arglet = "j";
    axisInIJK.applyMatrix4((volume as any).inverseMatrix).normalize();
    ...
    IJKIndex = Math.abs(
      Math.round(
        IJKIndex.applyMatrix4((volume as any).inverseMatrix).dot(axisInIJK)
      )
    );
    const base = [
      new Vector3(1, 0, 0),
    ...
    function sliceAccess(i: number, j: number) {
      ...
      const accessI =
        (iDirection as any).dot(base[0]) > 0
          ? si
          : (volume.xLength as number) - 1 - si;
      const accessJ =
        (jDirection as any).dot(base[1]) > 0
          ? sj
          : (volume.yLength as number) - 1 - sj;
      const accessK =
        (kDirection as any).dot(base[2]) > 0
          ? sk
          : (volume.zLength as number) - 1 - sk;
   ...
   */
  extractSlice(axis: string, index: number) {
  ...
  computeMinMax() {
    ...
    // buffer the length
    const datasize = (this.data as any).length;

    let i = 0;

    for (i = 0; i < datasize; i++) {
      if (!isNaN((this.data as any)[i])) {
        const value = (this.data as any)[i];
